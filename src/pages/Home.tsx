    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="bg-card px-4 py-2 border-b border-border">
        ...
        <Button
          ...
          variant="outline"
          className="w-full justify-start gap-2 h-10 text-base bg-card hover:bg-secondary border-2"
        >
      ...
      <div className="px-4 py-2">
        <Card className="p-2 flex items-center gap-3 hover:bg-secondary transition-colors cursor-pointer">
          ...
          <div className="flex-1">
            <h3 className="font-semibold text-sm">Chennai International Airport</h3>
            <p className="text-xs text-muted-foreground">Chennai - Trichy Hwy, Tambaram, Meenam...</p>
          </div>
        </Card>
      </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          {services.map((service, idx) => {
            ...
            return (
              <Card
                key={idx}
                className="relative p-2 hover:shadow-lg transition-all cursor-pointer border hover:border-accent"
                onClick={() => navigate("/location-search")}
              >
                {service.discount && (
                  <div className="absolute top-1 left-1 bg-success text-success-foreground text-[10px] font-bold px-1 py-0.5 rounded">
                    ⚡{service.discount}
                  </div>
                )}
                {service.promo && (
                  <div className="absolute top-1 left-1 bg-accent text-accent-foreground text-[10px] font-bold px-1 py-0.5 rounded">
                    Promo
                  </div>
                )}
