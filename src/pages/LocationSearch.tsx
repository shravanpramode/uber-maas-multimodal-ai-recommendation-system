...
import { ArrowLeft, MapPin, Clock, User, Plus, X } from "lucide-react";
...
const LocationSearch = () => {
  ...
  const recentPlaces = [
    ...
    { name: "Suburbian Floors, California Country", address: "SUBURBIAN FLOORS, Badauli Village, Sector 80,...", distance: "40 km", icon: "clock" },
          ...
          <div className="flex items-center gap-3 p-4 bg-background border-2 border-foreground rounded-2xl">
            ...
            <div className="flex-1 space-y-2">
              <Input
                ...
                className="border-0 p-0 text-sm font-medium focus-visible:ring-0 h-auto"
              />
              <Input
            ...
            {destinationLocation && (
              <Button variant="ghost" size="icon" className="h-6 w-6 rounded-full bg-foreground text-background hover:bg-foreground/90 hover:text-background -mr-1">
                <X className="w-4 h-4" />
              </Button>
            )}
      ...
      <div className="px-4 py-3">
        ...
        <div className="space-y-0">
          {recentPlaces.map((place, idx) => (
            <div
              key={idx}
              className="flex items-center gap-3 cursor-pointer hover:bg-secondary/50 py-3 transition-colors border-b border-border/50 last:border-0"
        ...
        <div className="space-y-0 mt-4 pt-4 border-t border-border">
          <Button
            variant="ghost"
            className="w-full justify-start gap-3 h-auto py-3 hover:bg-secondary/50"
            onClick={() => {}}
          >
